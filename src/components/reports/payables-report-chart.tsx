...
function ChartTooltip({ active, payload, label }: TooltipProps) {
  ...
  const paid = toNumberAmount(point?.paid ?? 0);
  ...
  return (
    <div className="rounded-xl border border-[#11203b]/[0.08] bg-white px-3.5 py-2.5 shadow-[0_4px_16px_rgb(17_32_59/0.1)]">
      ...
      <div className="mt-1.5 space-y-0.5 text-[12px] font-medium tabular-nums">
        <p className="text-[#6f9f87]">Pago {formatCurrency(paid)}</p>
...
export function PayablesReportChart({
  series,
  daily,
  totalAPagar,
}: PayablesReportChartProps) {
  ...
  const data = useMemo<ChartPoint[]>(
    () =>
      series.map((point) => ({
        ...
        paid: toNumberAmount(point.paid),
        ...
        {totalAPagar !== undefined ? (
          <>
            ...
            <span className="text-[11px] font-medium text-[#11203b]/42">
              <span className="text-[#c89b3c]/65">·</span> Total a pagar no
              período
            </span>
          </>
        ) : null}
