...
type PurchasesReportChartProps = {
...
export function PurchasesReportChart({
  series,
  daily,
  averageTicket,
}: PurchasesReportChartProps) {
  const isMobile = useIsMobile();
  const gradientUid = useId().replace(/:/g, "");
  const gradientId = `report-purchases-bar-${gradientUid}`;
          ...
          >
            <defs>
              <linearGradient id={gradientId} x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor={TR.douradoForte} />
                <stop offset="55%" stopColor={TR.dourado} />
                <stop offset="100%" stopColor={TR.douradoClaro} />
              </linearGradient>
            </defs>
            <CartesianGrid
            ...
            />
            <Tooltip
              cursor={{ fill: "rgb(200 155 60 / 8%)", radius: 8 }}
